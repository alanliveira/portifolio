...
function Cubie({ id, position, register }: { id: string; position: [number, number, number]; register: (id: string, node: THREE.Group | null) => void }) {
  return <group ref={(node) => register(id, node)} position={position}><RoundedBox args={[0.84, 0.84, 0.84]} radius={0.09} smoothness={3}><meshStandardMaterial color="#111827" roughness={0.34} metalness={0.15} /></RoundedBox>{position[2] === 0.9 ? <Sticker position={[0, 0, 0.426]} color={colors[0]} /> : null}{position[2] === -0.9 ? <Sticker rotation={[0, Math.PI, 0]} position={[0, 0, -0.426]} color={colors[3]} /> : null}{position[0] === 0.9 ? <Sticker rotation={[0, Math.PI / 2, 0]} position={[0.426, 0, 0]} color={colors[1]} /> : null}{position[0] === -0.9 ? <Sticker rotation={[0, -Math.PI / 2, 0]} position={[-0.426, 0, 0]} color={colors[4]} /> : null}{position[1] === 0.9 ? <Sticker rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.426, 0]} color={colors[2]} /> : null}{position[1] === -0.9 ? <Sticker rotation={[Math.PI / 2, 0, 0]} position={[0, -0.426, 0]} color={colors[5]} /> : null}</group>;
}

function Sticker({ position, rotation, color }: { position: [number, number, number]; rotation?: [number, number, number]; color: string }) {
  return <group position={position} rotation={rotation}><mesh><planeGeometry args={[0.62, 0.62]} /><meshStandardMaterial color={color} roughness={0.38} metalness={0.05} /></mesh></group>;
}
...
export function CubeFallback({ reducedMotion = false, positionX = 0, positionY = 0, scale = 1.1 }: { reducedMotion?: boolean; positionX?: number; positionY?: number; scale?: number }) {
  ...
  useFrame((state, delta) => {
    ...
    root.current.rotation.y = THREE.MathUtils.lerp(root.current.rotation.y, -0.42 + elapsed * 0.1 + state.pointer.x * 0.15, Math.min(1, delta * 1.8));
